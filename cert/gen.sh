...
for M in client server; do
	openssl genpkey -algorithm RSA -out cert/$M.key
	openssl rsa -pubout -in cert/$M.key -out cert/$M.pub
	openssl x509 -new -CA cert/ca.pem -CAkey cert/ca.key -force_pubkey cert/$M.pub -out cert/$M.pem -subj /O=tll-tls/OU=test/CN=$M -days 3650
	rm cert/$M.pub
done
